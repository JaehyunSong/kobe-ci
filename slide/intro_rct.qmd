...
8月19〜23日はNIIオンライン分析システムの大規模メンテナンス&法定停電により使用不可

* 19日は本講義の3日目
   * 多分、マッチングの回
* 19日の実施形態
   * **案1）**19日は3回目と4回目の理論編、29日に3回目と4回目の実習
   * **案2）**19日の実習は宋がやり方を見せるだけとする。
...
1. Discordに登録し、Discordを起動する。
   * アプリ版、Web版がある（どちらでも良い）。
1. 宋をフレンドとして追加
   * 宋のIDとタグは**Jaehyun#4730**
   ![](figs/01/discord.png)
...
* 2日目にRの解説は行うが、深入りはしない（できない）
   * <https://www.jaysong.net/micro-book/> $\leftarrow$ **全員**がこの資料レベルの内容を知っていれば2日目の内容を省略し、代わりに操作変数法について解説
