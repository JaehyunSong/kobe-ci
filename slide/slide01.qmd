---
title: "方法論特殊講義III"
subtitle: "第1回 因果推論の考え方とランダム化比較試験"
...
* Jared P. Lander. 2017. [*R for Everyone: Advanced Analytics and Graphics (2nd Edition),*](https://www.amazon.co.jp/dp/013454692X/) Addison-Wesley Professional.（[和訳有り](https://www.amazon.co.jp/dp/4839962154/)）
...
* **平常点**: 30%
   * 授業への参加度 (質問/発言)
   * [Slack](https://slack.com/intl/ja-jp/)での参加度も含む
* **期末レポート**: 70%
   * 提出方法、期限は講義最終日に告知
...
1. 研究のプロポーザル
   ...
   * 実現可能性があること
2. 既存研究の再現 (replication)
   1. 分析をそのまま再現
   2. 本講義で紹介した手法を用いて再現
3. 本講義で紹介した手法を用いた2次分析
...
[Morgan and Winship (2014) *Counterfactuals and Causal Inference: Methods And Principles For Social Research.* Cambridge.](https://www.amazon.co.jp/dp/1107694167)
...
* 内生性の原因
   ...
   3. 測定誤差
   4. セレクション・バイアスなど
* 最近の教科書はこれはすべてを**セレクション・バイアス**や欠落変数バイアスでまとめる傾向
...
::: {layout-ncol=2}
![](figs/01/Door1.jpg){fig-align="center"}

![](figs/01/Door2.jpg){fig-align="center"}
:::
...
bind_cols(Data[1:10, ], 
          tibble(X = rep("", 10)),
          Data[11:20, ]) %>%
  kableExtra::kbl(col.names = c("ID", "Female", "Age", 
                                "　",
                                "ID", "Female", "Age"))
```
:::
...
* 無作為割当で受講有無を決めると、「やる気」や「真面目さ」は Qunat と無関係 (**独立**) になる
   * $\Rightarrow$ 内生性がなくなる!
...
```{r, message = FALSE, echo = FALSE, fig.width = 2.5, fig.height = 0.8, dpi = 300, fig.align = "center"}
#| fig-width: 5
#| fig-height: 1.25
...
#| fig-height: 4
#| cahce: true
df <- read_dta("data/Day1_Data1.dta")
df %>%
...
* Freedman, David A. 2008. "Randomization Does Not Justify Logistic Regression," *Statistical Science
...
#| fig-width: 8
#| fig-height: 3.5
gg_df <- read_csv("data/Day1_Data3.csv")
gg_df <- gg_df %>%
